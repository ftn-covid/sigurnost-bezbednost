...
namespace Manager
{
	public class ServiceCertValidator : X509CertificateValidator
	{
		...
		{
			string serviceUser = Formatter.ParseName(WindowsIdentity.GetCurrent().Name);

			X509Certificate2 certificateServiceUser = CertManager.GetCertificateFromStorage(StoreName.My, StoreLocation.LocalMachine, serviceUser);

			if (!certificate.Issuer.Equals(certificateServiceUser.Issuer)) throw new Exception("Cert is not from valid issuer");
		}
	}
}
